...
The jackstraw method also performs well, recovering the optimal MSE more frequently than summation but also exhibiting a heavier tail of large MSEs.
The nature of the suboptimalities reflects the theoretical differences between the two approaches.
...
The jackstraw involves random permutations and relies on $p$-values that are inherently stochastic, which increases the variation in the chosen number of PCs.
Parallel analysis recovers the optimal MSE most often but has an even heavier right tail than the jackstraw.
This is attributable to the randomness of permuting an entire matrix.
